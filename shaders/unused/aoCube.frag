...
float cubeField( vec3 p ){
  ...
  float fieldSize = .4  + abs( sin( uParameterB.y) ) * 1.;
...
float sphereField( vec3 p ){
  ...
  float fieldSize = .4  + abs( sin( uParameterB.y + uTick) ) * .3;
  return opRepSphere( p , vec3(fieldSize ), .1 + uParameterB.x * .05 );
...
vec3 doCol( float lamb1 , float spec1  , float lamb2 , float spec2){
  ...
  return vec3(1. ) - smoothstep( .4 , .6 , lamb1 ) * vec3( 1. , 0. , 0. ) 
        - smoothstep( .4 , .6 , spec1 ) * vec3( 0. , 0. , 1. );
}
...
void main(){
  ...
  if( res.y > .5 ){
    ...
    col = vec3( AO * AO * AO );// * doCol( lamb1 , spec1 , lamb2 , spec2 );
  }

  float eVal = .01;
  if( vUv.x < eVal || vUv.x > 1. - eVal || vUv.y < eVal || vUv.y > 1. - eVal ){
